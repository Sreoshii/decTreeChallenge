...
**Zip codes must be modelled as categorical variables, specifically as non-ordinal categorical variables.**

From a real estate perspective, zip codes represent discrete geographic neighborhoods or regions. A zip code like 50015 doesn't have a meaningful numerical relationship to 50016—they're simply different locations. When we treat zip codes numerically, the decision tree algorithm makes splits like "zipCode < 50012.5", which has no practical interpretation for house prices. There's no inherent reason why houses in zip code 50013 should cost more or less than houses in zip code 50012 simply because one number is larger than another.

Our analysis confirms this problem: when zipCode was treated as numerical, it received 0.0% importance—essentially appearing irrelevant to price prediction. This happened because the numerical ordering of zip codes (50010, 50011, 50012, etc.) has no correlation with house prices. The algorithm correctly recognized that numerical splits on zip codes don't help predict prices.

Zip codes are **non-ordinal** categorical variables because:
- They have no natural ordering (unlike rating scales where 5 stars > 4 stars)
- The numerical value has no magnitude meaning (zip code 99999 isn't "better" than 10001)
- Each zip code represents a distinct category with potentially different price characteristics

**How should zip codes be encoded?** Zip codes should be treated as categorical variables, but **NOT always with one-hot encoding**. While one-hot encoding can work for small numbers of zip codes (like our analysis with 25 categories), it becomes problematic when dealing with hundreds of categories because it creates many sparse binary columns, leading to inefficient training, overfitting risk, and poor representation of geographic relationships. For datasets with many zip codes, better alternatives include target encoding, native categorical handling in modern libraries (CatBoost, LightGBM, HistGradientBoostingRegressor), or region-based encoding that groups zip codes into larger geographic areas.

### 2. R vs Python Implementation Differences: Why Do They Differ?

**This comparison focuses on R's `rpart` versus Python's `sklearn.tree.DecisionTreeRegressor`**—two traditional decision tree implementations. While `rpart` provides superior categorical variable handling compared to sklearn's `DecisionTreeRegressor`, it's important to note that modern Python libraries have since developed native categorical support that equals or exceeds `rpart`.

The fundamental difference between `rpart` and sklearn's `DecisionTreeRegressor` lies in how each implementation handles variable types:

**R's `rpart` advantage:**
- **Native categorical support**: R's `rpart` package recognizes categorical (factor) variables natively and can make multi-way splits on categorical variables, allowing it to compare all categories simultaneously
- **Efficient splitting**: Instead of creating binary splits like "zipCode == 50015" vs "everything else", R can split into groups of categories (e.g., "zipCode in {50015, 50016, 50020}" vs "other zip codes")
- **No dummy variable explosion**: R doesn't require one-hot encoding, which avoids creating many binary features and spreading importance across multiple dummy variables

**Python's `sklearn.tree.DecisionTreeRegressor` limitation:**
According to the official scikit-learn documentation at https://scikit-learn.org/stable/modules/tree.html, there is a critical weakness:
...
This documentation statement explicitly acknowledges that scikit-learn's DecisionTreeRegressor does not support categorical variables natively. All features must be treated as numerical/continuous, which is why we must use one-hot encoding as a workaround.

The documentation further states:

> "The decision tree algorithm in scikit-learn only supports numerical features. If you have categorical features, you need to encode them first using techniques like one-hot encoding or label encoding."

When we one-hot encode zip codes in Python, each zip code becomes a separate binary feature. This causes several problems:
1. **Importance dilution**: The importance of zip codes gets spread across 25+ binary features, making it hard to see the overall contribution
2. **Binary splits only**: Each split can only separate one zip code from all others, not create meaningful groups
3. **Tree complexity**: The tree becomes harder to interpret with many similar-looking features

**Which is better?** The answer depends on which implementations we're comparing:

- **R's `rpart` vs sklearn's `DecisionTreeRegressor`**: R's `rpart` clearly handles categorical variables better because it was designed with native categorical support, while sklearn's traditional `DecisionTreeRegressor` requires one-hot encoding workarounds.

- **R's `rpart` vs modern Python libraries**: Modern Python libraries such as CatBoost, LightGBM, XGBoost (with `enable_categorical=True`), and scikit-learn's `HistGradientBoostingRegressor` now provide native categorical handling that equals or exceeds `rpart`'s capabilities. These libraries avoid the one-hot encoding problem entirely and can handle categorical variables as efficiently as `rpart`.

**Conclusion**: While `rpart` has an advantage over sklearn's traditional `DecisionTreeRegressor` for categorical variables, this advantage does not extend to modern Python tree-based libraries, which now offer native categorical support on par with or superior to `rpart`. For practitioners choosing between R and Python for categorical-heavy modeling, the choice is less about categorical support and more about ecosystem preferences, since both languages now have strong categorical handling capabilities.

**Practical Recommendation**: For new projects involving categorical variables:
- If using **Python**, prefer modern libraries with native categorical support (CatBoost, LightGBM, XGBoost, or HistGradientBoostingRegressor) over sklearn's traditional DecisionTreeRegressor
- If using **R**, `rpart` remains an excellent choice with robust categorical handling
- If already using **sklearn**, consider upgrading to `HistGradientBoostingRegressor` for native categorical support within the same ecosystem
- The key takeaway: Always use implementations with native categorical support rather than relying on one-hot encoding workarounds, regardless of which language you choose

### 3. Suggestions for Better Categorical Handling in Python Decision Trees

While scikit-learn's traditional DecisionTreeRegressor has categorical limitations, the Python ecosystem has evolved to address this gap:

**Option 1: Histogram-Based Gradient Boosting (scikit-learn 0.22+)**

Scikit-learn now offers `HistGradientBoostingRegressor`, which natively supports categorical variables. According to the scikit-learn documentation (https://scikit-learn.org/stable/modules/ensemble.html#categorical-support), this newer implementation provides:

> "Native categorical support: the model can treat categorical features directly, without requiring one-hot encoding. This often results in better models with reduced preprocessing."

This represents a significant advancement because it avoids the one-hot encoding problem entirely while maintaining the scikit-learn API.

**Option 2: LightGBM**

LightGBM (Microsoft's gradient boosting framework) provides excellent categorical variable support. According to the LightGBM documentation (https://lightgbm.readthedocs.io/en/latest/Advanced-Topics.html#categorical-feature-support):
...
LightGBM uses an optimal split finding algorithm specifically for categorical variables, making it faster and often more accurate than one-hot encoding approaches.

**Option 3: CatBoost**

CatBoost (Yandex's gradient boosting library) was specifically designed with categorical variables in mind. The CatBoost documentation emphasizes:

> "CatBoost can work with categorical features without any preprocessing, using a novel method of processing categorical features that avoids the typical overfitting."

**Recommendation:**

For decision tree-based models in Python, I recommend using either:
1. **HistGradientBoostingRegressor** from scikit-learn for native categorical support while staying within the scikit-learn ecosystem
2. **LightGBM** for production applications requiring maximum performance and interpretability with categorical variables

These modern implementations represent the current state-of-the-art for handling categorical variables in Python tree-based models, moving beyond the limitations of the traditional DecisionTreeRegressor that requires one-hot encoding.
