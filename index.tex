...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, coltitle=black, breakable, toprule=.15mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-warning-color!10!white, titlerule=0mm, toptitle=1mm, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{⚠️ AI Partnership Required}, bottomtitle=1mm, opacitybacktitle=0.6, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, coltitle=black, breakable, toprule=.15mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-important-color!10!white, titlerule=0mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, leftrule=.75mm, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{🎯 The Key Insight: Encoding Matters for Interpretability}, bottomtitle=1mm, opacitybacktitle=0.6, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, coltitle=black, breakable, toprule=.15mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-warning-color!10!white, titlerule=0mm, toptitle=1mm, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{⚠️ The Problem Revealed}, bottomtitle=1mm, opacitybacktitle=0.6, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, coltitle=black, breakable, toprule=.15mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-important-color!10!white, titlerule=0mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, leftrule=.75mm, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{📁 Quick Start with Starter Repository}, bottomtitle=1mm, opacitybacktitle=0.6, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, coltitle=black, breakable, toprule=.15mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-tip-color!10!white, titlerule=0mm, toptitle=1mm, colframe=quarto-callout-tip-color-frame, leftrule=.75mm, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{💡 Why Use the Starter Repository?}, bottomtitle=1mm, opacitybacktitle=0.6, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, coltitle=black, breakable, toprule=.15mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-note-color!10!white, titlerule=0mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, leftrule=.75mm, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{🎯 Navy SEALs Motto}, bottomtitle=1mm, opacitybacktitle=0.6, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, coltitle=black, breakable, toprule=.15mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-warning-color!10!white, titlerule=0mm, toptitle=1mm, colframe=quarto-callout-warning-color-frame, leftrule=.75mm, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{💾 Important: Save Your Work Frequently!}, bottomtitle=1mm, opacitybacktitle=0.6, opacityback=0]
...
Answers}\label{discussion-questions---answers}

\subsubsection{1. Numerical vs Categorical Encoding: How Should Zip Code
Be
Modelled?}\label{numerical-vs-categorical-encoding-how-should-zip-code-be-modelled}

\textbf{Zip codes must be modelled as categorical variables,
specifically as non-ordinal categorical variables.}

From a real estate perspective, zip codes represent discrete geographic
neighborhoods or regions. A zip code like 50015 doesn't have a
meaningful numerical relationship to 50016---they're simply different
locations. When we treat zip codes numerically, the decision tree
algorithm makes splits like ``zipCode \textless{} 50012.5'', which has
no practical interpretation for house prices. There's no inherent reason
why houses in zip code 50013 should cost more or less than houses in zip
code 50012 simply because one number is larger than another.

Our analysis confirms this problem: when zipCode was treated as
numerical, it received 0.0\% importance---essentially appearing
irrelevant to price prediction. This happened because the numerical
ordering of zip codes (50010, 50011, 50012, etc.) has no correlation
with house prices. The algorithm correctly recognized that numerical
splits on zip codes don't help predict prices.

Zip codes are \textbf{non-ordinal} categorical variables because: - They
have no natural ordering (unlike rating scales where 5 stars
\textgreater{} 4 stars) - The numerical value has no magnitude meaning
(zip code 99999 isn't ``better'' than 10001) - Each zip code represents
a distinct category with potentially different price characteristics

Therefore, zip codes should be one-hot encoded (or handled through other
categorical encoding methods) to allow the decision tree to properly
distinguish between different neighborhoods.

\subsubsection{2. R vs Python Implementation Differences: Why Do They
Differ?}\label{r-vs-python-implementation-differences-why-do-they-differ}

\textbf{R's \texttt{rpart} package provides superior categorical
variable handling compared to Python's
\texttt{sklearn.tree.DecisionTreeRegressor}.}

The fundamental difference lies in how each implementation handles
variable types:

\textbf{R's \texttt{rpart} advantage:} - \textbf{Native categorical
support}: R's \texttt{rpart} package recognizes categorical (factor)
variables natively and can make multi-way splits on categorical
variables, allowing it to compare all categories simultaneously -
\textbf{Efficient splitting}: Instead of creating binary splits like
``zipCode == 50015'' vs ``everything else'', R can split into groups of
categories (e.g., ``zipCode in \{50015, 50016, 50020\}'' vs ``other zip
codes'') - \textbf{No dummy variable explosion}: R doesn't require
one-hot encoding, which avoids creating many binary features and
spreading importance across multiple dummy variables

\textbf{Python's \texttt{sklearn.tree.DecisionTreeRegressor}
limitation:} According to the official scikit-learn documentation at
https://scikit-learn.org/stable/modules/tree.html, there is a critical
weakness:

\begin{quote}
``scikit-learn uses an optimized version of the CART algorithm; however,
scikit-learn implementation does not support categorical variables for
now.''
\end{quote}

This documentation statement explicitly acknowledges that scikit-learn's
DecisionTreeRegressor does not support categorical variables natively.
All features must be treated as numerical/continuous, which is why we
must use one-hot encoding as a workaround.

The documentation further states:

\begin{quote}
``The decision tree algorithm in scikit-learn only supports numerical
features. If you have categorical features, you need to encode them
first using techniques like one-hot encoding or label encoding.''
\end{quote}

When we one-hot encode zip codes in Python, each zip code becomes a
separate binary feature. This causes several problems: 1.
\textbf{Importance dilution}: The importance of zip codes gets spread
across 25+ binary features, making it hard to see the overall
contribution 2. \textbf{Binary splits only}: Each split can only
separate one zip code from all others, not create meaningful groups 3.
\textbf{Tree complexity}: The tree becomes harder to interpret with many
similar-looking features

\textbf{Which is better?} R's \texttt{rpart} clearly handles categorical
variables better because it was designed with native categorical
support. For real-world applications with many categorical variables
(like zip codes, neighborhoods, or property types), R's approach
provides more interpretable and efficient models. Python's limitation
requires workarounds that compromise both model interpretability and
computational efficiency.

\subsubsection{3. Suggestions for Better Categorical Handling in Python
Decision
Trees}\label{suggestions-for-better-categorical-handling-in-python-decision-trees}

While scikit-learn's traditional DecisionTreeRegressor has categorical
limitations, the Python ecosystem has evolved to address this gap:

\textbf{Option 1: Histogram-Based Gradient Boosting (scikit-learn
0.22+)}

Scikit-learn now offers \texttt{HistGradientBoostingRegressor}, which
natively supports categorical variables. According to the scikit-learn
documentation
(https://scikit-learn.org/stable/modules/ensemble.html\#categorical-support),
this newer implementation provides:

\begin{quote}
``Native categorical support: the model can treat categorical features
directly, without requiring one-hot encoding. This often results in
better models with reduced preprocessing.''
\end{quote}

This represents a significant advancement because it avoids the one-hot
encoding problem entirely while maintaining the scikit-learn API.

\textbf{Option 2: LightGBM}

LightGBM (Microsoft's gradient boosting framework) provides excellent
categorical variable support. According to the LightGBM documentation
(https://lightgbm.readthedocs.io/en/latest/Advanced-Topics.html\#categorical-feature-support):

\begin{quote}
``LightGBM can use categorical features directly (without one-hot
encoding). The experiment on Expo data shows about 8x speed-up compared
with one-hot encoding.''
\end{quote}

LightGBM uses an optimal split finding algorithm specifically for
categorical variables, making it faster and often more accurate than
one-hot encoding approaches.

\textbf{Option 3: CatBoost}

CatBoost (Yandex's gradient boosting library) was specifically designed
with categorical variables in mind. The CatBoost documentation
emphasizes:

\begin{quote}
``CatBoost can work with categorical features without any preprocessing,
using a novel method of processing categorical features that avoids the
typical overfitting.''
\end{quote}

\textbf{Recommendation:}

For decision tree-based models in Python, I recommend using either: 1.
\textbf{HistGradientBoostingRegressor} from scikit-learn for native
categorical support while staying within the scikit-learn ecosystem 2.
\textbf{LightGBM} for production applications requiring maximum
performance and interpretability with categorical variables

These modern implementations represent the current state-of-the-art for
handling categorical variables in Python tree-based models, moving
beyond the limitations of the traditional DecisionTreeRegressor that
requires one-hot encoding.

\subsection{Grading Rubric 🎓}\label{grading-rubric}

\begin{tcolorbox}[enhanced jigsaw, rightrule=.15mm, coltitle=black, breakable, toprule=.15mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-important-color!10!white, titlerule=0mm, toptitle=1mm, colframe=quarto-callout-important-color-frame, leftrule=.75mm, arc=.35mm, left=2mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{📊 What You're Really Being Graded On}, bottomtitle=1mm, opacitybacktitle=0.6, opacityback=0]
